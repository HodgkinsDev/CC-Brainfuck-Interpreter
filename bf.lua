--Brainfuck for CraftOS
--Written by Jacob Hodgkins
...
local function readChar()
    local line = io.read("*l")  -- Read one line of input
    return string.sub(line, 1, 1)  -- Extract the first character
end
    ...
    while A <= length do
        ...
        if cmd == ">" then 
            ...
            if cellptr > cell_limit then
                ...
                cell[cell_limit] = 0 -- Initialize the new cell
            end
        elseif cmd == "<" then
            cellptr = cellptr - 1
            if cellptr < 1 then
